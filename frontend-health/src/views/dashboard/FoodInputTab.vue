...
const tabs: { key: string; label: string; comp: Component }[] = [
  { key: 'record', label: '记录三餐', comp: FoodInput },
  { key: 'search', label: '食物查询', comp: FoodSearch },
  { key: 'add', label: '自定义食物', comp: FoodAdd },
]

const activeTab = ref<string>((route.query.tab as string) || 'record')
const currentComponent = computed(() => tabs.find(t => t.key === activeTab.value)?.comp || FoodInput)
