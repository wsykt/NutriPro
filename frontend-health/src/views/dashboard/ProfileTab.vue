...
import DietaryProfile from './DietaryProfile.vue'

const route = useRoute()
const router = useRouter()

const tabs: { key: string; label: string; comp: Component }[] = [
  { key: 'info', label: '个人资料', comp: Profile },
  { key: 'dietary', label: '饮食偏好', comp: DietaryProfile },
]

const activeTab = ref<string>((route.query.tab as string) || 'info')
const currentComponent = computed(() => tabs.find(t => t.key === activeTab.value)?.comp || Profile)
