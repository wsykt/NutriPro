<template>
  <div class="home-page-dashboard min-h-full relative">
    <div class="dk-wrap relative z-10">
      ...
      <div class="stat-row" ref="statRowRef">
        ...
        <button class="stat-card" data-anim @click="go('/dashboard/muscle-chart')">
          <span class="stat-label"><Dumbbell class="w-3.5 h-3.5" style="color:#4E8D6E" /> 今日运动</span>
          <span class="stat-value">{{ todayExerciseMin ? todayExerciseMin + '' : '—' }}<i>{{ todayExerciseMin ? 'min' : '' }}</i></span>
          <span class="stat-sub">{{ todayExerciseMin ? '消耗 ' + todayExerciseKcal + ' kcal' : '暂无记录' }}</span>
          <span class="stat-bar"><i :style="{ width: todayExerciseMin ? '100%' : '8%', background: 'linear-gradient(90deg,#7FAE8E,#B8CE9E)', opacity: todayExerciseMin ? 0.5 : 0.2 }"></i></span>
        </button>
      </div>
...
const overview = ref({
  ...
  reportCount: 0,
  todayExerciseMin: 0, todayExerciseKcal: 0,
})
...
const todayExerciseMin = computed(() => overview.value.todayExerciseMin || 0)
const todayExerciseKcal = computed(() => overview.value.todayExerciseKcal || 0)
...
onMounted(async () => {
  const [diet, snap, reports, hist, arts, pred, exercise] = await Promise.allSettled([
    ...
    api.exercise.recordsRange ? api.exercise.recordsRange(today, today).catch(() => []) : Promise.resolve([]),
  ])
  ...
  if (hist.status === 'fulfilled') {
    ...
    if (Array.isArray(h)) {
      // 后端按记录日期降序返回（最新在前），图表按旧→新绘制，必须升序排序，
      // 否则趋势方向与"记录页"相反（曾导致首页显示上涨、记录页显示下降）
      const sorted = [...h].sort((a: any, b: any) => {
        const da = String(a?.date ?? a?.recordDate ?? '')
        const db = String(b?.date ?? b?.recordDate ?? '')
        return da < db ? -1 : da > db ? 1 : 0
      })
      weightHistory.value = sorted.map((x: any) => Number(x?.weight ?? x?.value ?? 0)).filter(Boolean)
      weightDates.value = sorted.map((x: any) => x?.date ?? x?.recordDate ?? '')
    }
  }
  if (arts.status === 'fulfilled') {
    const a = (arts.value as any)?.data ?? arts.value
    // 全量加载（接口无分页上限），让 filteredArticles 能按人群过滤命中当前用户人群的文章
    if (Array.isArray(a)) articles.value = a
    else if (a?.list) articles.value = a.list
  }
  ...
  if (exercise.status === 'fulfilled') {
    const list = (exercise.value as any)?.data ?? exercise.value
    const arr = Array.isArray(list) ? list : []
    overview.value.todayExerciseMin = arr.reduce((s: number, r: any) => s + (Number(r.durationMin) || 0), 0)
    overview.value.todayExerciseKcal = arr.reduce((s: number, r: any) => s + (Number(r.caloriesBurned) || 0), 0)
  }
