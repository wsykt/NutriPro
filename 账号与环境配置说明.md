...
本压缩包解压后，按本文档完成依赖安装与配置即可运行整套系统。压缩包内已预置演示数据、知识库与云端 API Key，**除向量模型外无需额外准备任何数据**。
...
|---|---|---|
| JDK | 8（1.8.x） | 后端编译与运行 |
| Maven | 3.6+ | 后端构建（压缩包内已附带 `apache-maven-3.9.6`，可直接使用） |
...
AI 服务的 RAG 检索依赖本地向量模型。**压缩包为控制体积未包含模型文件**（约 1.4GB），需手动下载。
